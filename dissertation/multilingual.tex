...
%% Make it clear that this was about SemEval but we're probably not going to
%% use it in Chipa because too clunky.
...
variables (see Figure \ref{fig:pentagram}) to solve the classification problem
for the five target languages of the SemEval 2013 task \cite{task10}. Given
English source sentences with a specific annotated focus word (from a small set
of possible focus words types), the task was to make a correct lexical
selection for translating into Dutch, French, German, Italian and Spanish,
matching the selections made by human annotators familiar with those languages.

In that work, the network has nodes that correspond to the distributions
...
is framed as a MAP inference problem, wherein we try to maximize the joint
probability of all five variables, given the evidence of the features extracted
from the source language sentence.
The inference process is carried out with loopy belief propagation
...
Our approach here for setting the weights in the Markov network requires, for
each edge in the graph, a parallel corpus for that language pair. It thus seems
less easily extensible than the classifier stacking approach, in which it is
clear how we can include information from many heterogeneous sources.
