...
\ref{fig:pentagram}) to solve the CL-WSD classification problem for the five
...
values corresponding to possible translations for the corresponding target
language. The distributions over these different translations are produced by
language-specific maximum entropy classifiers, roughly by simply running the
baseline Chipa system on the given input sentence.  The edges in the graph
represent pairwise potentials that are derived from the joint probabilities of
target language labels co-occurring in the available bitext.
