...
%% Maybe in fact we won't do SQUOIA? Can we just do Apertium instead? Would
%% that be easier to work with? Can we still run SQUOIA? Apertium is certainly
...
which in theory supports non-hierarchical phrase-based MT, but in practice
...
\section{Integration with SQUOIA}
...
We have also shown how Brown clusters, either when learned from a large
out-of-domain corpus or from a smaller in-domain corpus, provide useful
features for a CL-WSD task, substantially improving classification accuracy.
