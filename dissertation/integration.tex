...
The Moses decoder is written in C++, and ...
...
A following module moves syntactic information between the nodes and the chunks
in the tree, and finally, the tree is reordered according to the basic word
...
current state of the transformation (after rule-based lexical disambiguation),
extracts the original Spanish input text and finds lexical-selection ambiguities
that we can attempt to resolve with Chipa.  After finding potentially resolvable
ambiguities and getting recommendations from our CL-WSD classifiers, we
constrain the lexical choices based on the classifier outputs, and then update
the XML tree to pass the choices along to subsequent steps.
...
%% XXX: working here, is this mostly duplicated above?
