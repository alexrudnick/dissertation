...
Quechua with SQUOIA\cite{rios2015basic}, a primarily rule-based MT system that
was developed by Rios \emph{et al.} at the University of Zürich. These two
...
CL-WSD can be used with these translation systems. Afterwards, we look at
evaluating Chipa's effect on MT output in both of these use cases.
...
Our goal with integrating Chipa into these MT systems is to provide a prototype
for how we can add CL-WSD to different kinds of MT systems, especially focusing
on cases where there is a relatively small amount of text available for
training the target-side language model. Here  %% XXX working here
...
CL-WSD classifiers can make use of regularities that may not be obvious to
human rule-writers, providing improved lexical selection for any word type that
has adequate coverage in the training corpus.
...
candidate translations for each source word to Chipa ... %% XXX working here
...
\cite{rudnick:saltmil2014}, we had communicated between Chipa and a machine
...
quite low (lower than one BLEU point), and were not improved by adding Chipa's
CL-WSD. But this is a rather more difficult task, since we are attempting to
generate fully-inflected Quechua surface forms, rather than selecting
appropriate lemmas, and BLEU scoring requires us to match n-grams exactly,
which is made less likely by the rich morphology, including evidential markers,
of Quechua. Additionally, as Rios notes \cite[\S 5.9]{rios2015basic},
