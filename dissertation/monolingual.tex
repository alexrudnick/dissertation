...
The doc2vec approach is an extension of word2vec. Here during training, the
single hidden layer of the network used to predict words in a document contains
not only an embedding for a word type, but also an embedding learned for the
current document. Training proceeds by loading the current representation for
...
In practice, doc2vec is useful for producing vectors describing previously
unseen documents, which in our work and many text classification tasks will be
input sentences. At inference time, to produce a representation for a new
document, the embeddings for word types are held constant and we run the
learning process again (stochastic gradient descent or some other optimization
process) to infer a new embedding for the current document, such that it helps
predict words in the current context.

doc2vec thus provides a straightforward approach for learning representations
of new documents based on an unlabeled training corpus.  Here we use the
implementation of doc2vec provided by the gensim package \cite{rehurek-lrec},
and treat single sentences (or Bible verses) as documents, for which we can
