...
In either case, the result is a dense vector of dimensionality 400 (XXX either
justify this or try 200 as well), which we can pass to whichever machine
learning algorithm we like, as we did with the vectors based on word2vec,
previously.
...
Spanish-Guaraní and Spanish-Quechua, and with the most promising classifiers
from Chapter \ref{chap:baseline}.

\begin{itemize}
  \item POS tag features with baseline features
  \item POS tag and dependency parse features, with baseline features

  \item Europarl Brown clusters, with baseline features
  \item Wikipedia Brown clusters, with baseline features
  \item Europarl Brown clusters, with baseline features (context window)
  \item Wikipedia Brown clusters, with baseline features (context window)
  ...
  \item doc2vec embeddings trained on wikipedia (full sentence)
  \item doc2vec embeddings trained on wikipedia (context window)

  \item Combination: baseline features 
  \item Combination: pyramid word2vec with brown and syntactic
\end{itemize}

with adding syntactic features, adding features derived from Brown clusters,
some combinations of these; then the results for using neural embeddings by
themselves, followed by combining the previous features with the neural
embeddings.
