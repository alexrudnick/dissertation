...
making use of the literature on distributional semantics, but these will be
left to future work, either by this author or other researchers.
...
language pairs, and with larger data sets) to provide annotations.
...
relationships for each token available for feature extraction. The parser here
depends on the POS tags produced by FreeLing (thankfully they use the same tag
set), but as with any pipeline of NLP systems, this carries the risk that
tagging errors at early stages in the pipeline could propagate and cause
problems at the later stages of processing\footnote{Here we note this concern
...
tags \cite{PETROV12.274}; we map manually from the IULA tagset to universal
coarse tags.
...
\begin{itemize}
  ...
  source text be lemmatized? It seems as though part of the point of Brown
  clustering is to find abstractions over syntax, but perhaps lemmatization
  helps it find more semantic abstractions in practice.  If it's true, that's a
