...
For Spanish, we have not only off-the-shelf NLP tools, but also abundant
monolingual text. Given large amounts of Spanish-language text, we can use
...
%% we can also try topic models (say with MALLET) and word2vec representations
...
text, so adding more features is straightforward...

Notably, we can use POS tags, features extracted from parsers...
