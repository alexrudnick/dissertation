...
language pairs) to provide annotations.

For POS tagging, we run the open source FreeLing text analysis suite
\cite{padro12} on input sentences. FreeLing can perform a number of analysis
tasks for Spanish, including POS tagging, dependency parsing, lemmatization,
and named entity recognition, of which the latter two are part of the standard
preprocessing done for all experiments in this work. When all
of the text involved in an experiment is known beforehand, as in the
experiments reported in this chapter, then we can run FreeLing during the data
annotation step (see Section \ref{sec:datasetsandpreprocessing}) and simply
record its output as additional features. When running on novel test sentences,
as in server mode, we must run it on those individual sentences, just before
inference time.
Parsing was done with MaltParser \cite{Nivre06maltparser:a} \footnote{Available
at \url{http://maltparser.org/} ; in this work we use version 1.9.0 of the
parser, and the ``espmalt" pretrained model, which is available at
\url{http://www.iula.upf.edu/recurs01_mpars_uk.htm}, and was trained on the
IULA Treebank\cite{MARIMON12.519}, by researchers from the IULA group at
...
usage patterns, according to the bigram statistics of the corpus. The desired
number of ``leaf" clusters must be set ahead of time, as a tunable parameter.

We can then use this clustering to create more features for our classifiers, by
considering the clusters into which a word type falls as a tag. These
annotations describe a more abstract, less sparse representation than surface
forms or lemmas, hopefully providing useful semantic and syntactic
generalizations.
...
individual focus word. The received wisdom is that CBOW is more appropriate
when training on smaller data sets \footnote{According to the TensorFlow
tutorial on word2vec, available at
\url{https://www.tensorflow.org/versions/r0.7/tutorials/word2vec/index.html}}.
...
addition of features extracted from the available syntactic tools, Brown
clusters, neural word embeddings, document vectors. We do not try all possible
combinations of these, as that would simply be too many.
