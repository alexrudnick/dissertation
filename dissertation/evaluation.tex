...
similar languages, one may have to acquire Bible text for one's preferred
...
To support this work, we've developed \texttt{get\_verses.py}, a tool that uses
BeautifulSoup to parse the input HTML and navigate its structure, extracting
the verse identifiers and verse text as it goes. The output of
\texttt{get\_verses.py} we call a ``Bible file", which is full of tab-separated
values, one per line, in which (book, chapter, verse) tuples are paired with
the text of those verses.
...
Now on the target language side, we need to do a similar thing.  We use
\texttt{cut-corpus.pl} to select just the target side, in this case the
Guarani, and cdec's \texttt{tokenize-anything.sh} and \texttt{lowercase.pl} to
tokenize and lowercase the Guarani text.  Then we run Chipa's lemmatizer script
over the Guarani text. It calls the Paramorfo morphological analyzer on the
Guarani tokens, producing Guarani lemmas. In cases of morphological ambiguity,
we produce tokens that maintain that ambiguity, simply joining the possible
lemmas with a slash.
Once lemmatization is complete, we have produced the target-language lemmatized
text.
...
First, we have produced the lemmatized bitext corpus.
Second, we have generated the one-to-many word alignments between the two
sides of the bitext. These two files taken together constitute the ground truth
for the Chipa system -- the source words, and the subsequences of the target
text that are considered to be their translations. At test time, we will want
to predict those translations.
...
this annotated source-language corpus can contain all kinds of useful signals
for predicting translations.
