...
which will be extended in subsequent chapters.
...
the task faced by an MT system trying to perform lexical selection while
translating running text. We can train our classifiers for many different word
types, but We do not have sufficient training data to build classifiers for
every possible word. But this is a problem faced by data-driven NLP systems
broadly.
...
performance on these test sets provides a straightforward comparison
...
%% XXX this means we have to actually run Chipa on the SemEval test sets too.
...
various markup formats used for different translations in different languages.
...
most modern Protestant editions, and include slightly different chapters.
...
verses already identified by Loza and Mihalcea; it came basically ready for
alignment.
...
group, for example, the plural form of a word with its singular.

For English and Spanish, we can use the FreeLing suite of NLP tools
...
there are examples of this in Section~\ref{sec:exploring}.
...
At its heart, the Chipa software takes in a bitext corpus, marked up with
annotations for the source side, and the associated word alignments for that
bitext. It then trains CL-WSD classifiers for word types from the source
language on demand. 
...
Entropy), with the default scikit-learn settings.
...
sparser solutions and thus avoid overfitting.
...
appropriate for the application. We report classification accuracies for both
settings.
...
In order to gracefully include a variety of features and omnivorously consume
any available NLP tools for the source language, Chipa's input format and
feature functions allow for arbitrary annotations on source-language tokens.

Our input file format is one token per line, with fields split by tabs, and the
lemma of the token as the first field, the surface form of the token as the
second field. Following this are an arbitrary number of annotations for that
token, which may encode things such as part-of-speech, dependency relations,
Sentence boundaries are indicated by blank lines. An annotated sentence may
