...
\begin{itemize}
  ...
  sonqochay- kuqnin.} is mapped to \texttt{llapan munaqninkunamanta ma/mana
  huk/huklla ka/kanchu sonqochay - kuqnin}. \emph{manan} here could be an
  inflection of \emph{ma} ('certainly') or \emph{mana} ('no, none'). Similarly
  with \emph{huk} ('one, another one') / \emph{huklla} ('only, lonesome,
  single') and \emph{ka} ('yours') / \emph{kanchu} (a kind of bird).
  \item TODO: pick out nice Guarani example too.
\end{itemize}
\caption{Examples of morphological ambiguity in Quechua and Guarani,
having been run through AntiMorfo and ParaMorfo. This is the first sentence of
Lamentations 1:2.}
