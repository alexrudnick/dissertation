...
As discussed in Section \ref{sec:background-wsd}, for many years WSD systems
have been evaluated \emph{in vitro}, on test sets in which sentences are
hand-annotated with monolingual word senses, such as (typically) the WordNet
sense inventory. These senses were chosen fairly arbitrarily by human
lexicographers, and contain many useful and interesting distinctions, but they
do not necessarily make the distinctions most appropriate for a given task.
Monolingual WSD evaluations also often include both coarse-grained and
fine-grained distinctions, as some word senses identified by lexicographers are
...
hand-label our test sentences.
But for language pairs where we already have translated documents, this task
has in a sense already been performed by the translators.
%% XXX: working here.
...
side of the bitext, in this case the Spanish.  Then we call Freeling to do
basic analysis on the input Spanish text: we use Freeling for tokenizing,
lemmatizing and tagging \footnote{See the checked-in Freeling configuration
file for the exact settings used:
